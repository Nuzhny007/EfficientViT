//Based on safetensors.cpp from Carson Poole
#pragma once
...
namespace safetensors
{
	...
	std::unordered_map<std::string, TensorInfo> parse_safetensors_header_info(const char* data, size_t size)
	{
		if (size < 8)
	...
	inline std::unordered_map<std::string, torch::Tensor> load_safetensors(const std::string& filename, torch::Device device)
	{
		...
		if (!file || file.gcount() != static_cast<std::streamsize>(file_size))
			throw SafetensorsException("Failed to read file: " + filename);

		uint64_t header_size;
		std::memcpy(&header_size, buffer.data(), sizeof(uint64_t));
		if (is_big_endian())
		...
		auto tensor_infos = parse_safetensors_header_info(buffer.data(), file_size);
		...
		char* data_start = buffer.data() + 8 + header_size;
